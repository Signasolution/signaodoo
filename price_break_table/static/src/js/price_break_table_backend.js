...
export class PriceBreakTableBackend extends Component {
    ...
    setupQuantityListener() {
        // Écoute des changements de quantité dans les formulaires de vente
        const quantityInputs = document.querySelectorAll('input[name="product_uom_qty"]');
    ...
    async onRowClick(row) {
        ...
        const quantityInput = document.querySelector('input[name="product_uom_qty"]');
        if (quantityInput) {
...
// Enregistrement du composant pour le backend
registry.category("view_widgets").add("price_break_table", PriceBreakTableBackend);
